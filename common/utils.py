...
class Utils:
    ...
    def validate_request(
        ...
        if page is not None and (limit is not None or offset is not None):
            raise RestValidationError(
                "requesting a specific page is incompatible with limit and offset"
            )
        ...
        if query_type in ("projectable_properties",) and is_querystring_defined:
            raise RestInputValidationError(
                "projectable_properties requests do not allow specifying a query string"
            )
        ...
        for field in field_list:
            if field[0] == "profile":
                if field[1] == "=":
                    ...
                    raise RestInputValidationError(
                        "only assignment operator '=' is permitted after 'profile'"
                    )
            elif field[0] == "limit":
                if field[1] == "=":
                    limit = field[2]
